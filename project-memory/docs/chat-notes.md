...
- App icon assets `ClockWidget/Assets/AppIcon.ico` and `ClockWidget/Assets/AppIconSource.png` were regenerated from the user's supplied orange hourglass image. Black background was made transparent; ICO frames include 256/128/64/48/32/16. The 64/48/32/16 ICO frames were later rebuilt from a tighter crop so the hourglass reads larger in the Windows tray.
...
- User asked to add a second Pomodoro Stats tab for intensity by hour of day and today's Pomodoro list. The tab is named `Rhythm`; it shows a 24-hour focus-minutes chart and today's completed focus sessions. New completions now store `PomodoroFocusSessions` entries with completion timestamp and duration, written only on focus completion. Older daily aggregate stats cannot populate the hourly chart retroactively because they do not include timestamps.
- User asked to delete unnecessary Pomodoro entries from the `Rhythm` window. Today rows now have a red cross on the left; clicking it removes that timestamped session, decrements matching daily stats/history, saves once, and refreshes the stats window/widget.
- User clarified the `Rhythm` chart should support average/total and all-time/month/week/day views. Added in-window link controls for Total/Average and All time/Month/Week/Day; selected links are highlighted and disabled. Total sums focus minutes by hour; Average divides each hourly total by active days that have at least one timestamped Pomodoro session in the selected period. The latest selected mode/range is persisted on user click and restored next time.
- User reported that tab/menu buttons in `Pomodoro Stats` (`Overview`, `Rhythm`) and then also in Settings do not show the right border. Attempts to fix with an overlay border and then an explicit right-column `Rectangle` did not visibly solve it on the user's Windows render. The issue is left unresolved for now.
- Follow-up: `Pomodoro Stats` now avoids the problematic framed tab shape. `Overview` / `Rhythm` use underline-style text tabs with a bottom active indicator and hover fill, so there is no right border to clip. The Settings tab style still has the unresolved right-border issue.
- Follow-up for Settings: the framed Settings tab style still clipped on Windows even after adding internal spacing and moving the explicit right-edge line inward. Settings tabs were changed to the same underline-style text tabs as `Pomodoro Stats`, removing the right border entirely.
- Follow-up: user chose `Sessions` as the visible replacement for the `Rhythm` tab name in `Pomodoro Stats`. Internal `PomodoroRhythm*` code names remain unchanged for now because this was a UI text rename.
- User asked whether focus-minute values in the `Sessions` hourly bar chart can sit directly above each bar instead of all labels sharing one top line. Changed the hourly chart rendering so each hour column uses a bottom-aligned vertical stack containing its value label and bar, with a small reserved label area above the max bar height.
- User then asked to apply the same behavior to `Overview` -> `Monthly focus`. Changed monthly chart rendering so each month column uses a bottom-aligned vertical stack containing the Pomodoro count label and its bar, with a small reserved label area above the max bar height.
- User asked to change the `Sessions` subtitle from `Focus minutes by hour of day` to `Focus minutes / pomodoros by hour of day` and make the hourly columns switch between minutes and Pomodoro count. Added a persisted `Minutes` / `Pomodoros` unit selector. Minutes keep the existing per-minute distribution across hours; Pomodoro counts are grouped by completion hour. Existing Total/Average and Period filters continue to apply.
- User asked to add the same minutes/Pomodoros choice to the `Overview` bar chart. Added `Minutes` / `Pomodoros` links next to `Monthly focus`; the selected unit is persisted separately from the `Sessions` hourly chart. Monthly bars now use either `FocusMinutes` or `Count` for value/height while tooltips still show both.
- User asked whether `Sessions` -> `Today` can show one ripe red tomato beside the word `Today` for each completed Pomodoro today. Implemented small WPF-drawn tomato icons in the Today header, rebuilt from timestamped focus sessions so deleting a Today session updates the icons.
