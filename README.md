...
- Prevents duplicate widget instances and brings the existing widget forward when launched again.
...
- Tabbed settings window for background shade, opacity, border, weekday/date, precise padding, size, and clock font weight.
...
- Built-in and named presets can restore visual settings plus seconds/date visibility.
- Custom presets are labeled separately from built-ins; saving with a built-in name creates a custom override that can be reset.
- Settings can be imported or exported manually from the Settings window.
- Apply is enabled only when the current settings draft differs from the last applied state.
